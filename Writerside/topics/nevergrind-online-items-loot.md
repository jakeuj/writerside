...
- 檢視日期：`2026-05-03`
- 資料來源：NotebookLM 摘要、Fandom Loot / Items 頁、Nevergrind Wiki Magic Find Mechanics、Steam / SteamDB patch notes
...
Fandom Loot 頁目前把物品整理成藍、黃、紫、綠四大類；NotebookLM 摘要則提到橙色 legendary。這代表公開資料可能跨版本，判斷時要先看遊戲內 tooltip。

| 顏色 / 類型 | 常見意義 | 看裝重點 |
| ------ | ------ | ------ |
| 藍色 `Magic` | 1 到 2 個隨機詞綴 | 單一詞綴可能很高，適合補抗性、資源或特定缺口 |
| 黃色 `Rare` | 2 到 6 個隨機詞綴 | 詞綴組合對了，可能比固定裝更適合當前 build |
| 紫色 `Unique` | 固定詞綴種類，但數值可能波動 | 看固定效果、技能加成、部位與數值 roll |
| 綠色 `Set` | 固定詞綴與套裝 bonus | 單件未必最強，多件 bonus 才是重點 |
| 橙色 / Legendary / Mythical | NotebookLM 與新版資料提到的高階裝備概念 | 先以遊戲內顏色、名稱與 patch 說明為準，不要套舊 wiki 分類 |

舊 Fandom 頁特別提醒，magic 物品雖然詞綴少，但單一詞綴可能比同等級 rare 還高。這在補極端短板時很重要：你不一定要等完美黃裝，早期一件高抗性藍裝就能救你一條命。
...
NotebookLM 摘要提到 [Cryptic Paragon（Haniwa）](nevergrind-online-cryptic-paragon-haniwa.md)、`Charlatan's Crest` 這類社群常討論的高價值裝備，也在 Cleric / caster build 中反覆出現。這類名字值得記，但不要只靠名字判斷強度。
...
NotebookLM 摘要也提到 ethereal / indestructible 類特殊裝備概念。這類詞綴如果出現在目前版本，通常要優先看 tooltip：ethereal 的基礎性能、耐久風險、是否能靠 indestructible 抵消，是會直接影響是否值得投入符文的判斷點。
