...
1. 檢查 `Writerside/topics/Aram.md` 是否存在與是否有實質內容：
   - 若「檔案不存在」或內容仍為預設 placeholder（例如只有 `# Aram` / `Start typing here...`），則視為**首次建立**：
     - 以建議的基礎章節結構產生新檔案，例如：
       ...
       - Bug / 坑點整理
       - 如何搭配 Blitz / 其它工具說明
     - 並在檔案最開頭加上一段「資料覆蓋標記」（見 5.3 節），紀錄目前已處理的巴哈頁數與樓層。
...
2. 無論是首次建立或更新，都應維護 `Aram.md` 開頭的「資料覆蓋標記」區塊，使其反映目前中介 JSON 已涵蓋的巴哈頁數與樓層（見 5.3、5.4 節）。  
3. 用「局部補丁（apply_patch）」方式建立或更新 `Writerside/topics/Aram.md`，不要動到其他 Writerside 設定檔。

---

### 5.3 Aram.md 開頭的資料覆蓋標記格式
