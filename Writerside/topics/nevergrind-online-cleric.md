...
`Cleric` 是 Nevergrind Online 中最穩定的直接治療者：它穿 plate armor，擅長補血、buff、stun、魔法傷害與處理不死或召喚系敵人。把它只當「奶媽」會太窄；中後期的 Cleric 更像是一個能補、能暈、能撐、也能在特定條件下打出傷害的防禦型混合職。
...
<tldr>
<p><code>Cleric</code> 的核心價值是直接治療、plate armor、防禦 buff、stun 與救場。</p>
<p>高階玩法會把 Cleric 往神聖傷害、anti-undead / summoned、控場與 DPS 補位推進。</p>
<p>真正難點不是會不會補，而是 spirit / mana、aggro、施法 knockback 與何時停手。</p>
</tldr>

## 核心定位

Nevergrind Wiki 把 `Cleric` 定位成 defensive healer，並指出它擁有遊戲中很高的治療潛力。它的作戰重點包含：
...
這讓 Cleric 成為三大 healer 中最「穩」的選擇。Druid 比較偏進攻與自然控制，Shaman 比較偏 debuff 與續航；Cleric 則是救急、穩血、保隊伍下限。

## 三條天賦方向

官方 wiki 的 Cleric 頁列出三個 talent 方向：`Scion`、`Vestal`、`Arbiter`。可以先用下面方式理解：

| 天賦方向 | 主要傾向 | 適合情境 |
| ------ | ------ | ------ |
| `Scion` | 偏魔法傷害與 offensive caster | 想把 Cleric 推向 DPS、神聖傷害或特定怪物特攻 |
| `Vestal` | 偏治療與直接支援 | 想當穩定主補、強化群補與治療循環 |
| `Arbiter` | 偏防禦、審判、控場與生存 | 想提升承壓、降 aggro、stun 或 hybrid front-line 容錯 |

NotebookLM 摘要提到高階玩家會把 Cleric 往 DPS 化發展，特別是搭配神聖傷害、暈眩與對不死 / 惡魔類目標的加成。這個方向可以放在 `Scion` 或攻擊型配置裡理解，但實際配點仍要看當前版本技能說明。

## 重要技能概念

不要死背每一招，先理解 Cleric 的技能分工。若只是要查熱鍵、冷卻、施法時間與 Twloli 中文對照，可以直接看 [牧師技能參考（Twloli）](nevergrind-online-cleric-skills-twloli.md)。
...
SteamDB 的更新紀錄曾提到 `Imprecatory Psalm` 的行為調整：目標會決定它偏向攻擊怪物或治療隊伍，而不是同時做兩件事。這類紀錄提醒我們，Cleric 的天賦與技能互動會被版本調整；配點前最好看當前遊戲內說明。

## 牧師為什麼會變成 DPS

Cleric 的官方描述已經不是純治療，它本來就包含 stun、buff、healing、magical damage 與處理 tough situations 的 utility。高階玩法把 Cleric 推向 DPS，通常來自幾個原因：
...
但這不代表新手 Cleric 應該一開始就無腦打傷害。隊伍需要你救場時，你的第一責任仍然是讓隊伍活下來；DPS 化是在資源、aggro 與治療節奏都穩定後才成立。

## 三個主要風險

### Spirit / mana 被打乾

治療、buff、控制與傷害都會吃資源。長戰鬥中如果資源耗盡，Cleric 的價值會急速下降。
...
治療會製造威脅值，尤其在 tank 尚未建立穩定 aggro 時很容易出事。Cleric 雖然穿 plate armor，但被多怪盯上仍然會出問題。
...
施法者被攻擊時可能被打斷或延長施法時間。Cleric 的 plate armor 是容錯，不是免死金牌；如果被怪貼住，原本能救人的治療也可能因 knockback 來不及出手。

處理方式是預判而不是反應：血線快掉前先補防禦 buff，危險怪準備施法前先 stun，自己被盯上時先交保命或降威脅工具。

## 新手玩法建議

1. 先把自己定位成「穩定隊伍的人」，不要急著證明 Cleric 能打。
2. 每次進地城前確認常用 buff 是否補齊。
3. 開場等 tank，血線穩定後再補傷害。
4. 練習把 stun 用在危險怪，而不是看到技能亮就按。
5. 中期開始準備兩套思路：一套主補，一套偏輸出或特定地城 farm。

Cleric 的魅力在於它不是單薄的補血機器。玩得好時，你會同時在做四件事：讓隊友不死、讓怪打不出關鍵技能、讓自己不被 aggro 打崩，並在安全時間把傷害補上去。
...
如果目標是把 Cleric 往輸出推，可以接著看 [Nevergrind Online Scion 牧師極限輸出指南](nevergrind-online-cleric-scion-dps.md)，裡面會整理 `Scion` / `Arbiter` 混合點法、`Deliverance` burst window 與裝備方向。
...
- [SteamDB: Cleric talent update and talent itemization support](https://steamdb.info/patchnotes/12621770/)
