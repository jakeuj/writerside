...
- 延伸閱讀：[裝備收集路線](nevergrind-online-equipment-collection.md)、[鑑定（Identification）指南](nevergrind-online-identification.md)、[商人（Merchant）指南](nevergrind-online-merchant.md)、[賭博（Gambling）指南](nevergrind-online-gambling.md)
...
## 金幣（Gold）花在哪裡 {#gold-spending-priorities}
...
<note>
<p>FC2 loot 頁提到稀有兩手物理武器若 DPS 超過某個門檻，售價會變得穩定且高昂。這類數字很吃版本與 item level，筆記中保留「高 DPS 兩手武器值得看」這個原則，不把特定門檻寫成永久規則。</p>
</note>

## Unique / Set 的賣裝例外 {#unique-set-sell-exceptions}
...
完整操作可看 [鑑定（Identification）指南](nevergrind-online-identification.md) 與 [藥劑店（Apothecary）指南](nevergrind-online-apothecary.md)。
...
<warning>
<p>來源摘要提到 solo 玩家可用 <code>/players 5</code> 模擬滿隊難度與收益。這類指令是否存在或是否影響 gold，請以目前遊戲內 <code>/help</code>、酒館（Tavern） tips 與官方說明為準；沒有確認前，不要把它當成固定金幣策略。</p>
</warning>
...
## 金幣（Gold）的後期流向 {#gold-endgame-sinks}

當技能、背包、補給與基本裝備都穩定後，gold 才適合大量流向高風險系統：

| 系統 | 投入時機 | 注意 |
| ------ | ------ | ------ |
| [賭博（Gambling）](nevergrind-online-gambling.md) | 已知道要追 elite 飾品、Haniwa、Shako 或特定部位 | 高風險，先確認 quality 與圖示 |
| [符文（Runes）](nevergrind-online-runes.md) | 裝備會穿很久，或高 socket 工具裝已確定用途 | 一般鑲嵌通常視為不可逆 |
| [製作（Crafting）](nevergrind-online-blacksmith-crafting-recipe-research.md) | 已有正確 socketed base、runes 與 recipe 方向 | base、socket count、required level 都要符合 |
...
- [鑑定（Identification）指南](nevergrind-online-identification.md)
