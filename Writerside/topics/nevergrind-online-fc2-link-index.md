...
本次先按副檔名盤點本地鏡像，再決定哪些內容進公開攻略。結果是攻略主資料集中在 HTML；圖片、樣式與腳本只用來理解原站呈現，不直接搬入本站。

| 副檔名 | 數量 | 處理方式 |
| ------ | --: | ------ |
| `.html` | 106 | 逐一抽出頁名、主題、連結與可公開摘要；依主選單、職業頁、Unique 部位頁、Set 明細頁分類 |
| `.png` | 347 | 多為 item icon、class icon、talent screenshot；不複製進公開攻略，只在需要時保留「原站有圖示 / talent 圖」的描述 |
| `.jpg` | 90 | 多為職業 talent 圖與裝備例截圖；不重製、不搬圖，改寫成文字摘要與關鍵字 |
| `.js` | 3 | 原站互動、tab 與小工具用；只記錄 `dpscalc.html` 是工具頁，不把程式搬入攻略 |
| `.css` | 2 | 原站樣式；不進攻略內容 |
...
| 職業 build | [職業系統總覽](nevergrind-online-classes.md)、[FC2 職業 Build 摘要](nevergrind-online-fc2-class-build-index.md) 與各職業專頁 | 用來標示「高階方向」與裝備依賴，不當成新手唯一模板 |
...
| [各職の代表スキル](https://atelier3.web.fc2.com/ngo/selectlist.html) | 各職業代表技能速查 | 對應 [FC2 各職代表技能速查](nevergrind-online-fc2-signature-skills.md)；用來判斷裝備上的 tree / talent / skill roll |
| [各職の厳選ユニーク](https://atelier3.web.fc2.com/ngo/selectlist2.html) | 各職常見武器、防具、飾品 Unique 候選 | 對應 [FC2 各職嚴選 Unique 裝備速查](nevergrind-online-fc2-selected-unique-items.md)；適合當高階裝備查表入口 |
...
14 個職業頁的共通摘要、套裝路線與裝備例關鍵字已整理到 [FC2 職業 Build 摘要](nevergrind-online-fc2-class-build-index.md)；下表保留逐頁入口，方便回 FC2 查原始 talent 圖與裝備例。
