...
這篇是攻略首頁底下的系統地圖，只保留各系統之間怎麼互相影響。若你已經理解四大系統，下一步可以看 [進度路線與 FC2 攻略讀法](nevergrind-online-progression-roadmap.md)，把 `Superior`、`Mastery`、`Nightmare`、`Hell` 與 `Heroic` 串成實際養成順序。
...
- 物品與戰利品（Items & Loot）
  ...
  - 詳細稀有度、magic find、鑑定與商店策略可看 [物品與戰利品總覽](nevergrind-online-items-loot.md)，鑑定流程則可接著看 [鑑定（Identification）指南](nevergrind-online-identification.md)。
- 城鎮設施（Town - Edenburg（伊登堡））
  - 負責任務、補給、技能、鑑定、銀行、背包與裝備強化；完整入口可看 [Edenburg（伊登堡）城鎮設施總覽](nevergrind-online-edenburg.md)。
  - 酒館是接任務、查看排行榜與出發前 ready 的核心入口；可看 [酒館（Tavern）指南](nevergrind-online-tavern.md)。
  - 藥劑店負責補給、`Identify All` 與 caster / healer 向裝備；可看 [藥劑店（Apothecary）指南](nevergrind-online-apothecary.md)。
  - 鐵匠鋪負責重甲、重武器與 rune / crafting 類後期強化；可看 [鐵匠鋪（Blacksmith）指南](nevergrind-online-blacksmith.md)。
  - 商人負責背包擴張、皮甲與隨機庫存，是刷寶續航的後勤核心；可看 [商人（Merchant）指南](nevergrind-online-merchant.md)。
  - 每次地城結束後，角色成長大多在這裡完成。
...
| 系統 | 重點 | 延伸閱讀 |
| ------ | ------ | ------ |
| 稀有度（Rarities） | `Magic`、`Rare`、`Unique`、`Set`、`Legendary` 各自代表不同隨機性與固定效果 | [物品與戰利品總覽](nevergrind-online-items-loot.md) |
| 品質階級（Quality Tiers） | `Normal`、`Exceptional`、`Elite` 影響基礎性能與裝備階段 | [傳奇裝備（Legendary Items）清單](nevergrind-online-legendary-items.md) |
| 套裝（Set Items） | 綠色裝備，多件同套會啟用 `Set Bonus` | [套裝（Set Items）指南](nevergrind-online-set-items.md) |
| 符文（Runes） | 帶孔裝備可以鑲嵌 rune，永久補強特定性能 | [符文（Runes）指南](nevergrind-online-runes.md) |
| 賭博（Gambling） | 用 gold 嘗試定向取得特定部位或高階裝備 | 後期金幣出口；可看 [賭博（Gambling）指南](nevergrind-online-gambling.md) |
...
Edenburg（伊登堡）可以想成每次地城之間的整理台。打完一場回來，不只是賣垃圾，而是要把角色下一場的成功率往上推；如果想先看完整城鎮循環，直接看 [Edenburg（伊登堡）城鎮設施總覽](nevergrind-online-edenburg.md)。

| 設施 | 功能 | 使用時機 |
| ------ | ------ | ------ |
| 酒館（Tavern） | 接任務、前往地城、查看排行榜與 tips | 每次出發前確認任務、掉落偏好與隊伍目標；可看 [酒館（Tavern）指南](nevergrind-online-tavern.md) |
| 學院（Academy） | 學習或升級職業技能 | 金幣優先投入常用 rotation 技能 |
| 藥劑店（Apothecary） | 購買藥水、鑑定卷軸、使用 `Identify All` 類服務 | 地城後大量鑑定與補給；可看 [藥劑店（Apothecary）指南](nevergrind-online-apothecary.md) |
| 鐵匠鋪（Blacksmith） | 購買重型武裝、裝備升級、rune / crafting 相關功能 | 裝備確定會穿一段時間後再投入；可看 [鐵匠鋪（Blacksmith）指南](nevergrind-online-blacksmith.md) |
| 商人（Merchant） | 擴展背包、購買皮甲或一般商品 | 背包空間不足會直接降低刷寶效率；可看 [商人（Merchant）指南](nevergrind-online-merchant.md) |
...
- 想決定職業、定位或裝備可用性：看 [職業系統與裝備可用性總覽](nevergrind-online-classes.md)。
- 想把成長節點接到 `Hell` / `Heroic`：看 [進度路線與 FC2 攻略讀法](nevergrind-online-progression-roadmap.md)。
- 想規劃地城、怪物 traits 與任務：看 [地城冒險與任務攻略](nevergrind-online-dungeons.md)。
- 想整理掉落、鑑定、留裝與金幣效率：看 [物品與戰利品總覽](nevergrind-online-items-loot.md)。
- 想研究牧師後期刷區域：看 [牧師刷區域指南](nevergrind-online-cleric-farming-zones.md)。
