...
`Fahlnir Citadel`（法爾尼爾城堡）可以當成 Act IV 的高壓進階區域來看：敵人混合 undead、demon、mystical、giant 與 dragonkin，掉落目標也開始指向 70 級以上裝備。對 `Cleric` 來說，它不是最舒服的新手刷點，而是技能循環、抗性、資源回復與裝備都成形後，用 undead / demon 特攻去換高階收益的地方。

- 檢視日期：`2026-05-03`
- 分類：[Nevergrind Online 牧師刷區域指南](nevergrind-online-cleric-farming-zones.md)
- 相關 build：[Scion 牧師輸出指南](nevergrind-online-cleric-scion-dps.md)
...
- 資料來源：NotebookLM 摘要、來源資料整理、Nevergrind Wiki monster mechanics 與既有裝備資料整理
...
<p>Cleric 進場前要先解決 fire / lightning resist、spirit / mana sustain，以及 <code>Spirit Drain</code> 類抽資源壓力。</p>
...
| [Cryptic Paragon（Haniwa）](nevergrind-online-cryptic-paragon-haniwa.md) 在來源摘要中被列為 level 75 候選 | Cleric / caster 追逐的高階 weapon 目標與此階段 farm 期待接近 |
...
Fahlnir Citadel 的價值在於敵人類型夠高階，也夠混雜。這對 Cleric 是好消息，也是壞消息：undead / demon 讓 `Deliverance`、`Condemnation` 類技能有發揮空間，但非加成目標與危險 traits 也會逼你切換打法。

| 類型 | 來源摘要例子 | 對 Cleric 的意義 |
| ------ | ------ | ------ |
...
| `Mystical` | Gargoyle 類石像鬼 | 不一定吃 Cleric 的 undead / demon 特攻，優先看實際技能效率 |
| `Giant` | 巨人類目標 | 血量與物理壓力可能偏高，注意 threat 與位置 |
| `Dragonkin` | `Lord Yrrassolth` 類龍族 boss | 通常是區域後段或 boss 壓力來源，不能只靠 undead / demon 腳本處理 |

不要因為區域裡有 undead / demon，就把每一隻怪都當成 Cleric 特攻目標。Fahlnir 的重點是「挑對目標打爆發，遇到非加成或高風險 traits 就降速」。

## 需要特別警戒的 traits

| Trait / 風險 | 來源摘要例子 | 為什麼危險 | 應對 |
| ------ | ------ | ------ | ------ |
| `Spirit Drain` | `Baron Lucien` | 被攻擊時損失 spirit，會直接拆掉 Cleric / Druid 的治療與續航 | 降低貪輸出，保留 potion、stun 與逃生節奏 |
...
<warning>
<p>如果同時看到抽資源、降抗性、延長施法或降低治療效果，不要把 Fahlnir 當成純刷寶地圖硬衝。這種組合會讓 Cleric 同時失去輸出、治療與反應速度。</p>
</warning>
...
4. 裝備有基本 armor、HP、casting haste 與 all talents / Cleric 重要技能加成。
5. 背包、藥水與鑑定卷軸都準備好，避免高價掉落被迫丟棄。

對 Scion DPS Cleric 來說，常見節奏是先觀察 traits，再用 `Sacred Revelation` 或 `Force of Glory` 開短 stun window，接 `Deliverance` 處理 undead / demon 高價值目標，最後用 `Condemnation` 補 cone / 多目標傷害。若資源被抽乾，就立刻退回治療、保命與重整節奏。
...
- 高階 set / unique / legendary 候選，尤其是能補 Cleric burst、資源或 Hell 生存缺口的部位。
...
- [Nevergrind Online Scion 牧師極限輸出指南](nevergrind-online-cleric-scion-dps.md)
